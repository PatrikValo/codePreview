...
def cd(current, path):
    ...
    """
    alist_of_input = path.split('/')
    position = current
    length = len(alist_of_input)
    for index in range(length):
        if alist_of_input[index] == ".":
            continue
        elif alist_of_input[index] == "..":
            if position.parent != None:
                position = position.parent
        elif alist_of_input[index] in position.subdirectories:
            position = position.subdirectories[alist_of_input[index]]
        else:
            return None
    return position
...
def mkdirp(current, path):
    ...
    for index in range(length):
        if alist_of_input[index] == ".":
            ...
            a = Directory()
            a.parent = position
            position.subdirectories[alist_of_input[index]] = a
            position = position.subdirectories[alist_of_input[index]]
...
def find_n(current, pattern, alist):
    if current.subdirectories == {}:
        return alist
    else:
        for key, value in current.subdirectories.items():
            if pattern in key:
                alist += [value]
            alist = find_n(value, pattern, alist)
    return alist
